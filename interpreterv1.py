...
from typing import TypeGuard, Union, NoReturn, Self
...
FieldValueType = Union[int, StringWithLineNumber, bool, None, str, 'ObjectDefinition']
...
class Method:
    def __init__(self, name: StringWithLineNumber, method_params: tuple[StringWithLineNumber], expr: StatementType, nothing: Nothing) -> None:
        self.name = name
        self.params: dict[StringWithLineNumber, FieldValueType | Nothing] = {}  # name mapped to value
...
class ObjectDefinition:
    ...
    def __find_method(self, method_name: str, line_num: int) -> Method | NoReturn:
    ...
    def add_field(self, name: StringWithLineNumber, f: Field) -> None:
        self.__obj_fields[name] = f
        return

    # Interpret the specified method using the provided parameters
    def call_method(self, method_name: str | StringWithLineNumber, line_num: int, parameters=()) -> ReturnValue | NoReturn:
    ...
    def __run_statement(self, statement: StatementType) -> Union[Nothing, ReturnValue, int, bool, str, 'ObjectDefinition']:
        ...
        if is_print_statement(statement):
            result = self.__execute_print_statement(statement)  # Nothing
        elif is_input_str_statement(statement):
            result = self.__execute_input_str_statement(statement)  # Nothing
        elif is_input_int_statement(statement):
            result = self.__execute_input_int_statement(statement)  # Nothing
            ...
            result = self.__execute_set_statement(statement)  # Nothing
    ...
    def __execute_print_statement(self, args: PrintStatementType) -> Nothing:
        output_str = ''
        for a in args[1:]:
            if is_statement(a):
                # execute statement
    ...
    def __execute_input_str_statement(self, statement: InputStringStatementType) -> Nothing | NoReturn:
    ...
    def __execute_input_int_statement(self, statement: InputIntStatementType) -> Nothing | NoReturn:
        var_name = statement[1]
        input = self.interpreter.get_input()
        assert isinstance(input, str), "input isn't a str"
        if var_name in self.__current_method.params:
    ...
    def __execute_begin_statement(self, statement: BeginStatementType) -> Nothing | ReturnValue:
        for sub_statement in statement[1:]:
            assert is_statement(sub_statement)
            res = self.__run_statement(sub_statement)
            if isinstance(res, ReturnValue):
                return res
        return self.interpreter.nothing

    def __execute_set_statement(self, statement: SetStatementType) -> Nothing | NoReturn:
        var_name = statement[1]
        ...
        if isinstance(res, Nothing):
            ...
            assert is_StringWithLineNumber(var_name) and not isinstance(res, ReturnValue)
    ...
    def __parse_value(self, x: StatementType | StringWithLineNumber | str | bool | int | Self | Nothing | ReturnValue) -> (int | StringWithLineNumber | bool | str | Self | None | Nothing | ReturnValue | 'ObjectDefinition' | NoReturn):
        ...
        if is_statement(x):
            ...
            if x[0] == '"' and x[-1] == '"':
                return x[1:-1]
            return x
        elif isinstance(x, (bool, int, ObjectDefinition, Nothing)):
            return x
    ...
    def __execute_if_statement(self, statement: IfStatementType) -> Nothing | ReturnValue | NoReturn:
    ...
    def __execute_while_statement(self, statement: WhileStatementType) -> Nothing | ReturnValue | NoReturn:
        res = "_"
    ...
    def __execute_add_expression(self, expr: AddExpressionType) -> int | str | NoReturn:
    ...
    def __execute_arithmetic_expression(self, expr: ArithmeticExpressionType) -> int | NoReturn:
        a = self.__parse_value(expr[1])
        b = self.__parse_value(expr[2])
    ...
    def __execute_int_string_comparison_expression(self, expr: IntStringComparisonExpressionType) -> bool | NoReturn:
        a = self.__parse_value(expr[1])
        b = self.__parse_value(expr[2])
    ...
    def __execute_equality_comparison_expression(self, expr: EqualityComparisonExpressionType) -> bool | NoReturn:
        a = self.__parse_value(expr[1])
        b = self.__parse_value(expr[2])
    ...
    def __execute_bool_comparison_expression(self, expr: BoolComparisonExpressionType) -> bool | NoReturn:
        a = self.__parse_value(expr[1])
        b = self.__parse_value(expr[2])
    ...
    def __execute_bool_unary_expression(self, expr: BoolUnaryExpressionType) -> bool | NoReturn:
        a = self.__parse_value(expr[1])
        if not isinstance(a, bool):
            self.interpreter.error(ErrorType.TYPE_ERROR, line_num=expr[0].line_num)
        return not a

    def __execute_call_expression(self, expr: CallExpressionType) -> ReturnValue | NoReturn:
        call_str = expr[0]
        assert is_StringWithLineNumber(call_str) and is_StringWithLineNumber(expr[2]) and call_str.line_num is not None
    ...
    def __execute_new_expression(self, expr: NewExpressionType) -> 'ObjectDefinition' | NoReturn:
...
class ClassDefinition:
    ...
    def __init__(self, fields_or_methods: ClassMembersType, interpreter: 'Interpreter') -> None | NoReturn:  # https://stackoverflow.com/questions/33837918/type-hints-solve-circular-dependency
        self.my_methods = {}
        self.my_fields = {}
        self.interpreter = interpreter
    ...
    def instantiate_object(self) -> ObjectDefinition:
        ...
        for name, field in self.my_fields.items():
            obj.add_field(name, deepcopy(field))
        return obj
...
class Interpreter(InterpreterBase):
    ...
    def __discover_all_classes_and_track_them(self, parsed_program: ParsedProgramType) -> None | NoReturn:
...
def main():
    ...
    # print(type(StringWithLineNumber(1, 2)), type(StringWithLineNumber(1, 2) + StringWithLineNumber(1, 2)))
